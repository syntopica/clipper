...
test('a selection wins over everything else', async () => {
  const result = await extractContent(docFrom('blog.html'), '<p>picked text</p>')
  expect(result.extractor).toBe('selection')
  expect(result.html).toContain('picked text')
})

test('a normal article goes through Defuddle and drops nav and footer', async () => {
  const result = await extractContent(docFrom('blog.html'), null)
...
test('a page Defuddle rejects falls further down the chain', async () => {
  const result = await extractContent(docFrom('spa-shell.html'), null)
  expect(result.extractor).toBe('body')
  expect(result.html).toContain('Loading dashboard data')
})

test('extraction does not mutate the source document', async () => {
  const doc = docFrom('blog.html')
  const script = doc.createElement('script')
  script.textContent = 'window.tracked = true'
  doc.body.appendChild(script)

  await extractContent(doc, null)

  expect(doc.querySelector('nav')).not.toBeNull()
  expect(doc.querySelector('script')).not.toBeNull()
})

test('falls back to article when Defuddle rejects the page', async () => {
  const doc = new DOMParser().parseFromString(
    '<html lang="en"><body><article><p>short</p></article></body></html>',
    'text/html',
  )
  const result = await extractContent(doc, null)
  expect(result.extractor).toBe('article')
  expect(result.html).toContain('short')
})

test('falls back to main when there is no article', async () => {
  const doc = new DOMParser().parseFromString(
    '<html lang="en"><body><main><p>short</p></main></body></html>',
    'text/html',
  )
  expect((await extractContent(doc, null)).extractor).toBe('main')
})

test('falls back to innertext when the body has only text', async () => {
...
test('innertext does not reinterpret plain text as markup', async () => {
  const doc = new DOMParser().parseFromString('<html lang="en"><body></body></html>', 'text/html')
